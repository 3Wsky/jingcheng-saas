...
function normalizeOfficialProduct(sp) {
  ...
  const skuPrices = (product.skuPrices || []).map((s) => ({
    ...
    sbomCode: s.sbomCode || '',
    config: helperCleanText(s.config || ''),
    color: helperCleanText(s.color || ''),
    colors: Array.isArray(s.colors) && s.colors.length ? s.colors.map((c) => helperCleanText(c)).filter(Boolean) : (s.color ? [helperCleanText(s.color)] : []),
    image: s.image || ''
  }));
